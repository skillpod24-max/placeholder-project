...
-- Create enums
CREATE TYPE public.job_status AS ENUM ('draft', 'pending', 'in_progress', 'completed', 'on_hold', 'cancelled');
...
CREATE TABLE public.user_roles (
  ...
  CONSTRAINT user_roles_user_id_role_company_key UNIQUE (user_id, role, company_id)
);
...
-- Team workers junction table
CREATE TABLE public.team_workers (
  id uuid NOT NULL DEFAULT gen_random_uuid(),
  team_id uuid NOT NULL,
  worker_id uuid NOT NULL,
  added_at timestamp with time zone NOT NULL DEFAULT now(),
  CONSTRAINT team_workers_pkey PRIMARY KEY (id),
  CONSTRAINT team_workers_team_id_fkey FOREIGN KEY (team_id) REFERENCES public.teams(id) ON DELETE CASCADE,
  CONSTRAINT team_workers_worker_id_fkey FOREIGN KEY (worker_id) REFERENCES public.workers(id) ON DELETE CASCADE,
  CONSTRAINT team_workers_team_id_worker_id_key UNIQUE (team_id, worker_id)
);
...
CREATE POLICY "Company and vendor users can update vendor data"
...
USING (
  EXISTS (
    ...
    JOIN public.user_roles ur ON ur.company_id = v.company_id
    WHERE v.id = teams.vendor_id
    AND ur.user_id = auth.uid()
  )
);
...
-- RLS Policies for team_workers
CREATE POLICY "Users can view team workers in their companies"
...
USING (
  EXISTS (
    ...
    JOIN public.user_roles ur ON ur.company_id = v.company_id
    WHERE t.id = team_workers.team_id
    AND ur.user_id = auth.uid()
  )
);

CREATE POLICY "Vendors can manage their team workers"
ON public.team_workers FOR ALL
USING (
  EXISTS (
    SELECT 1 FROM public.teams t
    JOIN public.vendors v ON v.id = t.vendor_id
...
CREATE POLICY "Assigned vendors and workers can update job status"
...
CREATE POLICY "Vendors can manage tasks for their jobs"
...
CREATE POLICY "Assigned workers can update their tasks"
...
USING (
  EXISTS (
    ...
    JOIN public.user_roles ur ON ur.company_id = j.company_id
    WHERE jt.id = team_tasks.job_task_id
    AND ur.user_id = auth.uid()
  )
);
...
USING (
  EXISTS (
    SELECT 1 FROM public.team_workers tw
    JOIN public.workers w ON w.id = tw.worker_id
    WHERE tw.team_id = team_tasks.team_id
